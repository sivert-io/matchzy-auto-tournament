...
Only **STEAM_API_KEY** must be filled manually if you used `generate-env.mjs` (secrets are random).

| Variable | Notes |
|----------|--------|
| `SESSION_SECRET` | Random — generated by `generate-env.mjs` |
| `SERVER_TOKEN` | Same on all CS2 servers (MatchZy) |
| `DB_PASSWORD` | Postgres in stack |
| `STEAM_API_KEY` | **You must set** — Steam OAuth |
| `FRONTEND_BASE_URL` | `https://admin...` (OAuth redirects) |
| `API_BASE_URL` | Same as admin URL (webhooks + MP) |
| `PLAYER_PORTAL_URL` | `https://play...` (checkout returns) |
| `ORG_HOST` / `PLAYER_HOST` | Match DNS |
| `ORGANIZATION_ID` | Same as `ORG_SLUG` |
| `MERCADOPAGO_*` | If `registrationFeeCents > 0` |

Full list: `docs/ENV.md`
