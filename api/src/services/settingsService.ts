...
class SettingsService {
  ...
  async setSetting(key: AppSettingKey, value: string | null): Promise<void> {
    ...
    if (value !== null) {
      ...
      if (key === 'matchzy_admins_url') {
        // Allow either an absolute URL or a relative API path (e.g. /api/public/admins.json).
        const normalized = this.normalizeMatchzyAdminsUrl(trimmed);
        await db.setAppSettingAsync(key, normalized);
        log.success('MatchZy Enhanced admins URL updated');
        return;
      }

      if (key === 'matchzy_admins_refresh_seconds') {
        const parsed = Number(trimmed);
        if (!Number.isInteger(parsed)) {
          throw new Error('matchzy_admins_refresh_seconds must be an integer');
        }
        // 0 disables periodic polling (plugin will refresh on server init + match load).
        const clamped = parsed <= 0 ? 0 : Math.max(10, Math.min(3600, parsed));
        await db.setAppSettingAsync(key, String(clamped));
        log.success(`matchzy_admins_refresh_seconds updated to ${clamped}`);
        return;
      }
  ...
  async getMatchzyAdminsUrl(): Promise<string | null> {
    const value = await this.getSetting('matchzy_admins_url');
    if (!value) return null;
    const trimmed = value.trim();
    return trimmed !== '' ? trimmed : null;
  }

  async getMatchzyAdminsRefreshSeconds(): Promise<number> {
    const value = await this.getSetting('matchzy_admins_refresh_seconds');
    // Default to "no periodic polling" (plugin refreshes on server init + match load).
    if (!value) return 0;
  ...
  private normalizeMatchzyAdminsUrl(v: string): string {
    const s = v.trim();
